window.onload = function() 
{
    var buttons = document.getElementsByTagName('span'), // Select all buttons
     ...
     for (var i = 0; i < buttons.length; i += 1) 
        { // Add onclick events to buttons
            ...
            } else if (buttons[i].innerHTML === 'F') {
            buttons[i].addEventListener("click", CtoF(i));
            } else if (buttons[i].innerHTML === 'C') {
            buttons[i].addEventListener("click", FtoC(i));
    ...
    */
        ...
        */
        ...
        }button.addEventListener('click',Submit,addValue);
    ...
    function calculate(i) 
    {
        return function() 
        {
            if (equation.length == 0) { // If nothing in array, do nothing
                ...
                if (isNaN(p)) 
                {   
                     answer = eval('0'+equation.join(''));
                }
                else
                {
                     answer = eval(p+equation.join(''));
                }
